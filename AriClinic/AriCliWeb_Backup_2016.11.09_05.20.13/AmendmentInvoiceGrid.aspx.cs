...
public partial class AmendmentInvoiceGrid : System.Web.UI.Page 
{
    ...
    protected void RadGrid1_ItemDataBound(object sender, Telerik.Web.UI.GridItemEventArgs e)
    {
        if (e.Item is GridCommandItem)
        {
            ...
            if (pat != null)
                imgb.OnClientClick = "NewAmendmentInvoiceRecordInTab();";
        }
        if (e.Item is GridDataItem)
        {
            ...
            command = String.Format("reportAmendmentInvoice({0});", id);
            imgb.OnClientClick = command;
            // assign javascript function to edit button
            imgb = (ImageButton)e.Item.FindControl("Edit");
            if (pat == null)
                command = String.Format("return EditAmendmentInvoiceRecord({0});", id);
            else
                command = String.Format("return EditAmendmentInvoiceRecordInTab({0});", id);
            imgb.OnClientClick = command;
    ...
    protected void RefreshGrid(bool rebind)
    {
        if (pat == null && cus == null)
            RadGrid1.DataSource = ctx.AmendmentInvoices.OrderByDescending(x => x.InvoiceDate);
        else
        {
            //if (pat != null)
            //    RadGrid1.DataSource = pat.Customer.Invoices;
            RadGrid1.DataSource = cus.AmendmentInvoices.OrderByDescending(x => x.InvoiceDate);
        }
