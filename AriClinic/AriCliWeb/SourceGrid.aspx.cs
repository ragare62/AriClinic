...
public partial class SourceGrid : System.Web.UI.Page 
{
    ...
    {
        // load grid data
        RadGrid1.DataSource = CntAriCli.GetSources(ctx);
    }
    ...
    protected void RadGrid1_ItemDataBound(object sender, Telerik.Web.UI.GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            command = String.Format("return EditSourceRecord({0});", id);
            imgb.OnClientClick = command;
            ...
            command = String.Format("ariDialog('Sources','{0}','prompt',null,0,0)", message);
            imgb.Visible = per.Create;
        }
    }
    ...
    protected void RadAjaxManager1_AjaxRequest(object sender, AjaxRequestEventArgs e)
    {
            ...
            if (e.Argument == "yes")
            {
                if (Session["DeleteId"] != null)
                {
                    try
                    {
                        sourceId = (int)Session["DeleteId"];
                        Source ser = (from s in ctx.Sources
