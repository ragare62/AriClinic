...
public partial class ProcedureGrid : System.Web.UI.Page 
{
    ...
    {
        // load grid data
        RadGrid1.DataSource = ctx.Procedures;
    }
    ...
    protected void RadGrid1_ItemDataBound(object sender, Telerik.Web.UI.GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            name = gdi["Name"].Text;
            if (callNumber > 0) call = String.Format("Procedure{0}", callNumber);
            ...
            command = String.Format("return EditProcedureRecord({0});", id);
            imgb.OnClientClick = command;
        ...
        if (typeOfControl.Equals("System.Web.UI.WebControls.ImageButton"))
        {
            ...
            switch (imgb.ID)
            {
                ...
                case "Edit":
                    break;
                case "Delete":
                    Procedure taxt = (from t in ctx.Procedures
                                    where t.ProcedureId == id
                                    select t).FirstOrDefault<Procedure>();
                    ctx.Delete(taxt);
                    ctx.SaveChanges();
                    RefreshGrid();
                    break;
