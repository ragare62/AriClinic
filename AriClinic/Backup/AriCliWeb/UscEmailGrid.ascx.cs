...
namespace AriCliWeb
{
    public partial class UscEmailGrid : System.Web.UI.UserControl
    {
            ...
            if (typeOfControl.Equals("System.Web.UI.WebControls.ImageButton"))
            {
                ...
                switch (imgb.ID)
                {
                    ...
                    case "Edit":
                        break;
                    case "Delete":
                        Email eml = (from em in ctx.Emails
                                     where em.EmailId == id
                                     select em).FirstOrDefault<Email>();
                        ctx.Delete(eml);
                        ctx.SaveChanges();
                        RefreshGrid(true);
                        break;
        ...
        protected void RadGrid1_ItemDataBound(object sender, Telerik.Web.UI.GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
                ...
                command = String.Format("return EditEmailRecord({0});", id);
        ...
        public void RefreshGrid(bool rebind)
        {
            ...
            {
                case "hccom":
                    RadGrid1.DataSource = hc.Emails;
                    break;
                case "clinic":
                    RadGrid1.DataSource = cl.Emails;
                    break;
                case "patient":
                    RadGrid1.DataSource = pat.Emails;
                    break;
                case "customer":
                    RadGrid1.DataSource = cus.Emails;
                    break;
                case "professional":
                    RadGrid1.DataSource = prof.Emails;
                    break;
            }
            // I must rebind?
