...
public partial class InsuranceGrid : System.Web.UI.Page 
{
    ...
    {
        // load grid data
        RadGrid1.DataSource = ctx.Insurances;
    }
    ...
    protected void RadGrid1_ItemDataBound(object sender, Telerik.Web.UI.GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            // backgroud color for internal insurances
            if (gdi["Internal"].Text == "True")
            {
            ...
            command = String.Format("return EditInsuranceRecord({0});", id);
            imgb.OnClientClick = command;
        ...
        if (typeOfControl.Equals("System.Web.UI.WebControls.ImageButton"))
        {
            ...
            switch (imgb.ID)
            {
                ...
                case "Edit":
                    break;
                case "Delete":
                    Insurance ins = (from s in ctx.Insurances
                                     where s.InsuranceId == id
                                     select s).FirstOrDefault<Insurance>();
                    ctx.Delete(ins);
                    ctx.SaveChanges();
                    RefreshGrid();
                    break;
